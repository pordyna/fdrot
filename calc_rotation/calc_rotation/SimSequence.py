...
form c_rotation import one_step_extended_pulse  # ignore this error somehow
...
class SimSequence:
    ...
    """
    def __init__(self, files: FilesList, iteration: Tuple[int,int,int],  first_slice: Tuple[int,int],
                 last_slice: Optional[Tuple[int, int]] = None, full_slice_length: Optional[int] = None) -> None:
        """ """
        if len(iterations) > 1 and last_slice is None:
            raise TypeError('`last_slice`, can\'t be None, if there is more than one iteration in the sequence.')
        if len(iterations) > 2 and full_slice_length is None:
    ...
    def get_data(self, steps: Union[int, Sequence[int], str], make_contiguous: bool = True)-> MutableSequence[np.ndarray]:
        ...
        for ii,  step in enumerate(steps):
            data[ii] = self.files.open(self.step_to_iter(step))
